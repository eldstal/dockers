...
def parse_input(args):
  ...
  DOCKER_CMD = f'''docker run --rm --name codeql-docker -v "{SRC}:/opt/src" -v "{OUTPUT}:/opt/results" -e "LANGUAGE={LANG}" -e "FORMAT={FORMAT}" -e "QS={QS}" -e "THREADS={THREADS}" codeql'''
...
def main():
  ...
  parser.add_argument('-j', '--threads', nargs='?', default=16, type=int, help='Number of threads to evaluate queries.')
